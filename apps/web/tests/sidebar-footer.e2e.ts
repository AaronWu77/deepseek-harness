...
it('stacks footer contributions above settings in expanded and collapsed sidebars', async () => {
  ...
  try {
    ...
    for (const collapsed of [false, true]) {
      await page.setContent('<style>' + css + '</style><div style="width:' + String(collapsed ? 56 : 300) + 'px;height:600px"><div class="' + className('root') + (collapsed ? ' ' + className('collapsed') : '') + '"><div style="flex:1"></div><div class="' + className('footArea') + '"><div class="' + className('footerActions') + '"><button id="planner" style="box-sizing:border-box;width:' + (collapsed ? '36px' : '100%') + ';height:40px;flex-shrink:1">Plan</button><div id="quota" style="box-sizing:border-box;min-width:0;width:' + (collapsed ? '36px' : '100%') + ';height:' + String(collapsed ? 36 : 160) + 'px">Quota</div></div><div class="' + className('settingsArea') + '"><button id="settings" style="height:36px">Settings</button></div></div></div></div>')
      ...
      await negative.evaluate((node) => { (node as HTMLStyleElement).remove() })
    }
  } finally {
    await browser.close()
  }
