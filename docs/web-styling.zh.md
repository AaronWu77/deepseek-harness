...
- Aurora 壳层在按 `--dsw-glass-frame-gap` 内缩的三栏窗口后面铺设 `--dsw-glass-canvas`。窗口使用 `--dsw-glass-frame-radius`、半透明 `--dsw-glass-window` 渐变与 prominent 高度，但不加背景滤镜：窗口直接位于画布上。会话列和右侧列同理使用 `--dsw-glass-scrim`。其他背后有内容的悬浮表面（包括侧栏层、输入框和 dockkit 面板）将 `--dsw-glass-fill*` 背景与匹配的 `--dsw-glass-blur*` 滤镜配对，并将 `--dsw-elevation-stroke-color` 重绑为 `--dsw-glass-stroke`。共享的紧凑菜单则使用上一条中的菜单专用填充与滤镜。
- 玻璃表面同时声明 `-webkit-backdrop-filter` 与 `backdrop-filter` 时，前缀声明放在前面：客户端 CSS 压缩器只保留最后一条声明，而 Chromium 会忽略只剩前缀的结果。ui-theme 的 [backdrop-filter spec](../packages/client/ui-theme/tests/backdrop-filter-styles.client.spec.ts) 检查顺序。存在 `position: fixed` 后代时，将玻璃绘制在独立的底层背景上；在它们的容器上使用背景滤镜会改变包含块，使全视口浮层被限制在容器内。
