...
- 高层级表面（菜单、浮层、对话框、面板、悬浮按钮、输入框）设 `border: 0` 并使用 `box-shadow: var(--dsw-elevation-panel)`、`var(--dsw-elevation-prominent)` 或输入框专用的 `var(--dsw-elevation-soft)`（更大模糊、更低透明度）：0.5px 发丝描边是第一层投影，`--dsw-elevation-stroke-color` 可按表面或状态重绑或抑制描边。不得将 `--dsw-alias-border-*` border 与 lv/elevation 投影配对——ui-theme 的 elevation spec 会拒绝；状态色 border（warn 面板）保持真 border。 悬浮在环境画布之上的表面——壳层各列、输入框，以及共享的菜单/对话框外壳与停靠套件面板——取 `--dsw-glass-fill*` 背景并配一档 `--dsw-glass-blur*` backdrop 滤镜，同时把 `--dsw-elevation-stroke-color` 重绑为 `--dsw-glass-stroke`，让发丝线读作玻璃边缘。会话列与右侧列改取 `--dsw-glass-scrim`：它们直接坐在画布上，做模糊只会模糊自己身后的画布。壳层外面套一扇浮动窗：舞台铺环境画布，并把三栏网格按 `--dsw-glass-frame-gap` 内缩，网格元素（`[data-shell-window]`）带 `--dsw-glass-frame-radius`、半透明的 `--dsw-glass-window` 渐变与 prominent 高度。窗口同样不做 backdrop 滤镜，理由与中列相同。写前缀属性在前、标准属性在后——客户端 bundle 的 CSS 压缩会把这一对合并为最后一条声明，顺序颠倒会让产物里只剩 `-webkit-backdrop-filter`，而 Chromium 报告它不受支持、静默忽略；ui-theme 的 [backdrop-filter spec](../../packages/client/ui-theme/tests/backdrop-filter-styles.client.spec.ts) 会跨全部包样式表强制这个顺序。不要把玻璃画在承载 `position: fixed` 后代的容器上：生效的 `backdrop-filter` 会让该元素成为它们的包含块，于是挂在玻璃列里的全视口浮层会把 `inset: 0` 解析成那一列、被压成一列的宽度。承载这类浮层的列应把玻璃画在内容之下、绝对定位的独立背景层上。
