...
Workspace selection uses `uiWorkspace.openWorkspace` to prepare the target and commit navigation. The selected label stays visible while the Session and Workspace list projections settle; a failed request keeps the composer inert with a localized retry action, while a committed open enables input before a lagging Workspace membership refresh. Draft text and attachments move in its synchronous preparation callback only while that request is current; later navigation or owner disposal leaves the original draft intact.
