...
`design-platform.css` owns the glass surface suite and the ambient canvas. `--dsw-glass-fill`/`--dsw-glass-fill-raised`/`--dsw-glass-fill-thick`/`--dsw-glass-fill-sunken` are the translucent fills a surface picks by elevation — the thick tier backs large content panels, where the fill has to carry readability the shell tiers delegate to the canvas, `--dsw-glass-stroke` and `--dsw-glass-highlight` are its hairline and specular edge, `--dsw-glass-blur`/`--dsw-glass-blur-strong`/`--dsw-glass-blur-thin` are the three `backdrop-filter` tiers, and `--dsw-glass-scrim` is the translucent sheet the conversation column paints so body text keeps its contrast over the ambient colour. `--dsw-aurora-1` through `--dsw-aurora-4` are four drifting colour fields and `--dsw-aurora-drift` is their period; the shell paints them and the glass surfaces blur them, which is what makes `backdrop-filter` visible at all. `--dsw-glass-canvas` is the static fallback for a host that renders no aurora layer. `base.css` declares the motion tokens those surfaces animate on: `--dsw-motion-spring`, `--dsw-motion-duration`, `--dsw-motion-duration-fast`, and `--dsw-motion-duration-slow`.
