...
`ctx.uiWorkspace.openSession(id)` 会选中会话，并让主区域返回会话界面；这两项构成一次 UI 导航操作，即使目标会话已经是当前会话也同样执行。`openWorkspace(id, beforeOpen?)` 成功打开结果时解析为 `true`，被后续导航替代时解析为 `false`；`forkSession(id)` 遵循相同的提交规则。新会话使用 `openWorkspace`，启动时的自动选区也使用同一导航取消信号，因此晚到的自动结果不会覆盖手动选择的工作区。可选的同步准备回调仅对仍有效的工作区请求执行，因此过期请求不会搬移 composer 草稿。后续导航或所有者释放会阻止晚到的 UI 提交，但不取消底层会话创建。选中失败时保留当前全局面板。会话行读取 `usePanelInfo`，在全局面板活跃时不显示会话选中样式；仅把焦点移到搜索框或目录选择器不会离开该面板。
