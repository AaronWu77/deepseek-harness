...
Saved user defaults from the retired `code` preset id are read as `ptc`, the renamed preset, so an upgrade does not block unnamed session creation. This migration applies only to the saved default; an explicitly named `code` id is still not part of the roster.
