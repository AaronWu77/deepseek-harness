...
Moving the glass off the column also retires the column's stacking context. No shell column declares a `z-index` any more: a column with one confines every `position: fixed` overlay mounted inside it, and the settings dialog — mounted in the sidebar footer and declaring `z-index: 1000` — lost to the conversation column's own `z-index`, painted under the transcript, and took no pointer events, so Settings could not be clicked at all and its translucent cover read as the dialog itself being transparent. `.window` is the single stacking context the shell needs for the ambient-canvas ordering; the sidebar's glass layer orders against its content by tree order instead of a negative `z-index`; and an [AppFrame stylesheet spec](../../../../packages/client/ui-layout/tests/app-frame-styles.client.spec.ts) pins the rule, because a jsdom render has no layout and no paint to catch it.
