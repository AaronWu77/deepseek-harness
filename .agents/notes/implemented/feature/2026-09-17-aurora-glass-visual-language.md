...
[AppFrame](../../../../packages/client/ui-layout/src/client/AppFrame.tsx) splits into a stage and a floating window. The stage paints the canvas and takes `--dsw-glass-frame-gap` as padding; `.window` is the real three-column grid, inset by that gap, with `--dsw-glass-frame-radius`, a translucent `--dsw-glass-window` gradient, and the prominent elevation. The window deliberately carries no `backdrop-filter` — it sits directly on the ambient canvas, where a blur has no visible work to do and would only make the window a containing block for `position: fixed` descendants. The stage renders the ambient canvas: five absolutely positioned spans at `inset: -18%` behind the window, which is raised to `z-index: 1`. Each field paints one aurora token and drifts on its own multiple of the base period (1, 1.21, 1.47, 0.89, 1.63), so the fields never resynchronize and the loop has no visible seam. Only `transform` is animated, which keeps the fields on the compositor, and `prefers-reduced-motion` stops them. The fields are sized above the viewport so neighbouring fields overlap; a field that covers only a corner leaves the canvas neutral wherever no other field reaches.
...
The ambient canvas deliberately carries no `filter: blur()`: a radial gradient with transparent stops is already soft, while a gaussian over five viewport-sized layers would force five large offscreen textures for no visible gain.
...
**Animating `background-position` on a single gradient layer.** That repaints the whole canvas on the main thread every frame. Five composited transform layers cost one transform each.
...
One cosmetic artifact is open: at some drift phases a soft vertical tonal seam is visible in the transcript column where the pink field's gradient edge crosses the reading area. Flattening that field's background removes it, so it is an edge artifact of the large radial-gradient layer rather than a layout or clipping fault; the field's stops were softened and `will-change: transform` dropped without eliminating it.
