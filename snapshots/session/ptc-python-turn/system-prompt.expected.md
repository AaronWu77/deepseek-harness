...
- Keep the program valid Python. Markdown, JSON, YAML, HTML, and other file contents are data, not source: quote or JSON-serialize them before placing them in a dict or tool argument; never paste raw file text after `:`. Prefer a declared file-reading binding when one is available.
